    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.lesson-categories.update', $category) }}" class="space-y-4">
                        @csrf
                        @method('PUT')

                        <div>
                            <label for="name" class="block text-sm font-medium">名称</label>
                            <input id="name" name="name" type="text" class="mt-1 w-full border rounded p-2" value="{{ old('name', $category->name) }}" required>
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" class="mt-1 w-full border rounded p-2" rows="3">{{ old('description', $category->description) }}</textarea>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input id="sort_order" name="sort_order" type="number" class="mt-1 w-full border rounded p-2" value="{{ old('sort_order', $category->sort_order) }}" min="0" max="100000" required>
                                @error('sort_order')<p class="text-sm text-red-600">{{ $message }}</p>@enderror
                            </div>
                            <div class="flex items-center space-x-2 mt-6">
                                <input type="hidden" name="is_active" value="0">
                                <input id="is_active" name="is_active" type="checkbox" value="1" class="rounded" @checked(old('is_active', $category->is_active))>
