                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.stores.store') }}" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input name="name" type="text" class="mt-1 w-full border rounded p-2" value="{{ old('name') }}" required>
                            @error('name')<p class="text-sm text-red-600">{{ $message }}</p>@enderror
                        </div>

                        <div>
                            <label class="block text-sm font-medium">住所</label>
                            <textarea name="address" class="mt-1 w-full border rounded p-2" rows="2" required>{{ old('address') }}</textarea>
                            @error('address')<p class="text-sm text-red-600">{{ $message }}</p>@enderror
                        </div>

                        <div>
                            <label class="block text-sm font-medium">電話番号</label>
                            <input name="phone" type="text" class="mt-1 w-full border rounded p-2" value="{{ old('phone') }}" required>
                            @error('phone')<p class="text-sm text-red-600">{{ $message }}</p>@enderror
                        </div>

                        <div>
                            <label class="block text-sm font-medium">アクセス情報</label>
                            <textarea name="access_info" class="mt-1 w-full border rounded p-2" rows="2">{{ old('access_info') }}</textarea>
                        ...
                        <div>
                            ...
                            <input name="google_map_url" type="url" class="mt-1 w-full border rounded p-2" value="{{ old('google_map_url') }}">
                            @error('google_map_url')<p class="text-sm text-red-600">{{ $message }}</p>@enderror
                        </div>

                        <div>
                            <label class="block text-sm font-medium">駐車場情報</label>
                            <textarea name="parking_info" class="mt-1 w-full border rounded p-2" rows="2">{{ old('parking_info') }}</textarea>
